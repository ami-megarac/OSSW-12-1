...
mctp_iodata iodata_read;
mctp_iodata iodata_write;
mctp_iodata iodata_reset;
...
#endif
{
	...
	switch (cmd)
	{
		...
		case MCTP_PCIE_IOCTL_READ:
			...
			if(copy_from_user(&iodata_read,(void *)param, sizeof(mctp_iodata)))
				return -EFAULT;
			
			ret = pmctppcie_hal->pmctppcie_hal_ops->downstream_mctp_pkt(&iodata_read);

			// Update the iodata structure and copy_to_user
			if(copy_to_user((void *)param, &iodata_read, sizeof(mctp_iodata)))
				return -EFAULT;
		break;
		
		case MCTP_PCIE_IOCTL_WRITE:

			if(copy_from_user(&iodata_write,(void *)param, sizeof(mctp_iodata)))
				return -EFAULT;

			ret = pmctppcie_hal->pmctppcie_hal_ops->upstream_mctp_pkt(&iodata_write);

			if(copy_to_user((void *)param, &iodata_write, sizeof(mctp_iodata)))
				return -EFAULT;
		break;
        ...
        case MCTP_CHECK_RESET:
                    if(copy_from_user(&iodata_reset,(void *)param, sizeof(mctp_iodata)))
		        return -EFAULT;
                    ret = pmctppcie_hal->pmctppcie_hal_ops->mctp_check_reset(&iodata_reset);
		    // Update the iodata structure and copy_to_user
		    if(copy_to_user((void *)param, &iodata_reset, sizeof(mctp_iodata)))
		        return -EFAULT;
                break;
        default:
