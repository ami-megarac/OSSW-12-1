...
#define AST_SCU_CLK_SELECT				0x08
#define AST_SCU_ECLK_SOURCE_MASK		(3 << 2)
#define AST_SCU_CLK_VIDEO_SLOW_EN		(1 << 31)
...
#define AST_SCU_MISC_CTRL_D2_PLL		0x00000010 /* bit 4 */
#define AST_SCU_MISC_EN2D_CRT_REG		0x00000080 /* bit 7 */

static int set_dac_src_disp_out(struct astfb_dispsrc* src)
{
	uint32_t reg;
	uint32_t val;
	...
	case DISPSRC_VGA:
		...
		break;
	case DISPSRC_FB:
		val = (AST_SCU_MISC_CTRL_DVO_SRC | AST_SCU_MISC_CTRL_DAC_SRC_CRT | AST_SCU_MISC_EN2D_CRT_REG);
		break;
	default:
		printk("set_dac_src_disp_out error: Invalid src\n");
		return -1;
	}
	
	iowrite32(AST_SCU_UNLOCK_MAGIC, (void * __iomem)(AST_SCU_VA_BASE + AST_SCU_LOCK_KEY)); /* unlock SCU */
	reg = ioread32((void * __iomem)(AST_SCU_VA_BASE + AST_SCU_MISC_CTRL)); /* read display src */

	reg &= ~(AST_SCU_MISC_CTRL_DAC_SRC | AST_SCU_MISC_CTRL_DVO_SRC | AST_SCU_MISC_EN2D_CRT_REG);
	reg |= val;

	iowrite32(reg, (void * __iomem)(AST_SCU_VA_BASE + AST_SCU_MISC_CTRL)); /* write display src */
	iowrite32(0, (void * __iomem)(AST_SCU_VA_BASE + AST_SCU_LOCK_KEY)); /* lock SCU */

	return 0;
}

static int get_dac_src_disp_out(struct astfb_dispsrc* src)
{
	u32 reg = 0;
	reg = ioread32((void * __iomem)(AST_SCU_VA_BASE + AST_SCU_MISC_CTRL)); /* read display src */
	reg &= (AST_SCU_MISC_CTRL_DAC_SRC | AST_SCU_MISC_CTRL_DVO_SRC);
	if( (AST_SCU_MISC_CTRL_DVO_SRC | AST_SCU_MISC_CTRL_DAC_SRC_CRT) == reg )
		src->val = DISPSRC_FB;
	else
		src->val = DISPSRC_VGA;
...
int astfb_init_hw(struct device *dev, struct astfb_info *dinfo)
{
	...
	iowrite32(AST_SCU_UNLOCK_MAGIC, (void * __iomem)(AST_SCU_VA_BASE + AST_SCU_LOCK_KEY)); /* unlock SCU */
#if defined(FEATURE_ENABLE_BMC_OWN_VGA_ACCESS)
	reg = ioread32((void * __iomem)(AST_SCU_VA_BASE + AST_SCU_MISC_CTRL));
#if defined(SOC_AST2500)
	reg &= ~(AST_SCU_MISC_CTRL_VGA_REG);
	reg |= (AST_SCU_MISC_CTRL_DAC_SRC_CRT | AST_SCU_MISC_CTRL_DVO_SRC | AST_SCU_MISC_EN2D_CRT_REG); /* set source to Graphics CRT */
#else
...
#endif
	// set up clk sel.
	reg = ioread32((void * __iomem)(SCU_CLK_SELECT_REG));// printk("fb, 1, init, clk_sel: %X\n", reg);
	reg &= ~((3 << 2) | (7 << 28) | (1 << 31));
	reg &= ~(1 << 8);
	iowrite32(reg, (void * __iomem)(SCU_CLK_SELECT_REG));udelay(100);
	reg = ioread32((void * __iomem)(SCU_CLK_SELECT_REG));// printk("fb, 2, init, clk_sel: %X\n", reg);

	/* enable clock of Graphics Display controller */
	reg = ioread32((void * __iomem)(SCU_CLK_STOP_REG));// printk("fb, 1, init, clk_sotp: %X\n", reg);
	reg &= ~(0x00000400);
	iowrite32(reg, (void * __iomem)(SCU_CLK_STOP_REG));
	reg = ioread32((void * __iomem)(SCU_CLK_STOP_REG));// printk("fb, 2, init, clk_sotp: %X\n", reg);
	udelay(10);


	/* reset Graphics Display controller */
	reg = ioread32((void * __iomem)(SCU_SYS_RESET_REG));// printk("fb, 1, init, sys_reset: %X\n", reg);
	reg &= ~(0x00002000);
	iowrite32(reg, (void * __iomem)(SCU_SYS_RESET_REG));

	/* reset 2D engine */
	reg = ioread32((void * __iomem)(SCU_SYS_RESET_REG));// printk("fb, 3, init, sys_reset: %X\n", reg);
	reg &= ~(0x00000080);
	iowrite32(reg, (void * __iomem)(SCU_SYS_RESET_REG));

	// set up sys reset ctrl reg2
	reg = ioread32((void * __iomem)(SCU_SYS_RESET_CTL_REG2));// printk("fb, 1, init, sys_reset ctrl2: %X\n", reg);
	reg &= ~(1 << 5);
	iowrite32(reg, (void * __iomem)(SCU_SYS_RESET_CTL_REG2));udelay(100);
...
static int astfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case AMIFB_SET_DISPSRC:
		...
		{
			printk("ASTFB_SET_DFB_DACSRC: Error copying data from user!\n");
			return -EFAULT;
		}

		if(set_dac_src_disp_out(&dispsrc) < 0)
		{
			printk("ASTFB_SET_DFB_DACSRC: Invalid data from user!\n");
			return -EINVAL;
		}
	...
	case AMIFB_GET_DISPSRC:
		...
		get_dac_src_disp_out(&dispsrc);

		if (copy_to_user((void*)arg, (void*)&dispsrc.val, sizeof(u32)))
		{
			printk("ASTFB_SET_DFB_DACSRC: Error copying data to user!\n");
			return -EFAULT;
		}
...
static int astfb_alloc_bigphysarea(struct astfb_info *dinfo)
{
	...
	printk(KERN_INFO "FB: got physical memory pool for size (%d on %p virtual/%08lx bus)\n", AST_FRAME_BUF_SZ, (unsigned long*)dinfo->frame_buf, (unsigned long)dinfo->frame_buf_phys);
...
static int astfb_drv_probe(struct device *dev)
{
	...
	dinfo->var.xres = dinfo->var.xres_virtual = 800;
	dinfo->var.yres = dinfo->var.yres_virtual = 600;
