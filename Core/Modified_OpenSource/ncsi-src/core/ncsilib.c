...
DisplayLinkStatus(NCSI_IF_INFO* info, UINT32 Link, UINT8 local_verbose)
{
	...
	{
		printk(KERN_DEBUG "NCSI(%s): Auto Negotiation Enabled\n",info->dev->name);
		if (Link & LINK_AUTO_NEG_COMPLETE)
			printk(KERN_DEBUG "NCSI(%s): Auto Negotiation Complete\n",info->dev->name);
		else
			printk(KERN_DEBUG "NCSI(%s): Auto Negotiation Not Complete\n",info->dev->name);
	}

	if (Link & LINK_PARALLEL_DET_FLAG)
		printk(KERN_DEBUG "NCSI(%s): Link Partner does not support Auto Negotiation \n",info->dev->name);

	if (Link & LINK_TX_FLOW_FLAG)
		printk(KERN_DEBUG "NCSI(%s): Pause Flow Control enabled on TX \n",info->dev->name);

	if (Link & LINK_RX_FLOW_FLAG)
		printk(KERN_DEBUG "NCSI(%s): Pause Flow Control enabled on RX \n",info->dev->name);

	if (Link & LINK_SERDES)
	{
		printk(KERN_DEBUG "NCSI(%s): SerDes is being used \n",info->dev->name);
		return;
	}
	...
	if (Link & LINK_PARTNER_1000TFD)
		printk(KERN_DEBUG "NCSI(%s): Link Partner supports 1000Base-T Full Duplex \n",info->dev->name);
	if (Link & LINK_PARTNER_1000THD)
		printk(KERN_DEBUG "NCSI(%s): Link Partner supports 1000Base-T Half Duplex \n",info->dev->name);
	if (Link & LINK_PARTNER_100T4)
		printk(KERN_DEBUG "NCSI(%s): Link Partner supports 100Base-T4\n",info->dev->name);
	if (Link & LINK_PARTNER_100TXFD)
		printk(KERN_DEBUG "NCSI(%s): Link Partner supports 100Base-TX Full Duplex \n",info->dev->name);
	if (Link & LINK_PARTNER_100TXHD)
		printk(KERN_DEBUG "NCSI(%s): Link Partner supports 100Base-TX Half Duplex \n",info->dev->name);
	if (Link & LINK_PARTNER_10TFD)
		printk(KERN_DEBUG "NCSI(%s): Link Partner supports 10Base-T Full Duplex \n",info->dev->name);
	if (Link & LINK_PARTNER_10THD)
		printk(KERN_DEBUG "NCSI(%s): Link Partner supports 10Base-T Half Duplex \n",info->dev->name);
...
CheckIfAEN(NCSI_IF_INFO *info, struct sk_buff *skb)
{
	...
	if (verbose & SHOW_AEN_INFO) 
		printk("NCSI(%s): AEN Received From (%x.%x) : ", info->dev->name, GET_PACKAGE_ID(aen->hdr.OrgChannelID), GET_CHANNEL_ID(aen->hdr.OrgChannelID));
	...
	switch (aen->AEN_Type)
	{
		case AEN_TYPE_LINK_STATUS_CHANGE:
			...
			for(i=0;i<info->TotalChannels;i++)
			{
				if (info->ChannelInfo[i].Enabled == 1){
					Channel = MK_CH_ID(info->ChannelInfo[i].PackageID,info->ChannelInfo[i].ChannelID);
					if (Channel != aen->hdr.OrgChannelID){
						printk( "NCSI(%s): Channel 0x%x doesn't enable, Channel enabled 0x%x\n",info->dev->name, aen->hdr.OrgChannelID,Channel); 
						return 1;
					}
				}
			}			
		...
		case  AEN_TYPE_CONFIG_REQUIRED:
			...
			printk(KERN_DEBUG "NCSI (%s): AEN Config Required\n",info->dev->name);
			return 1;
...
ProcessResponseCode(NCSI_IF_INFO *info,UINT16 ResponseCode, UINT16 ReasonCode)
{
	switch (be16_to_cpu(ResponseCode))
	{
		case  COMMAND_COMPLETED:
			if(verbose & SHOW_MESSAGES)
				printk("NCSI(%s): %s : Completed.\n", info->dev->name, Cmd2Str(info));
	...
	if ((ReasonCode & 0xFF00) == 0)
	{
		...
		switch (ReasonCode&0xFF)
		{
			...
			case  REASON_MAC_ADDR_ZERO:
				...
				break;
			case  REASON_INDEPENDENT_CNTRL_NOT_SUPPORTED:
				printk("Independent transmit and receive enable/disable control is not supported\n");
				break;
			default:
				printk("Unknown Reason\n");
				break;			
...
int Check_PackageID(NCSI_IF_INFO *info, UINT8 PackageID)
{
	int HwArbit = 0;
...
//	if((selected_pkg != PackageID) && (info->TotalChannels != 0))
	if((selected_pkg != PackageID))
	{
		if(selected_pkg != MAX_PACKAGE_ID + 1)
		{
			NCSI_Issue_DeSelectPackage(info, selected_pkg);
		}
		
		NCSI_Issue_SelectPackage(info,PackageID,HwArbit);
	}
	...
	UINT32 Speed, UINT32 Duplex)
{
	...
	if (retval != NCSI_ERR_SUCCESS)
	{
		//! Set link is failed, so set the correct value of speed, duplex and autoneg by get link command.
		if (NCSI_ERR_SUCCESS == NCSI_Issue_GetLinkStatus(info, PackageID, ChannelID, &LinkStatus ))
		{
//			DisplayLinkStatus(info, LinkStatus, 0);
...
NCSI_Issue_UserCommand (NCSI_IF_INFO *info,UINT8 PackageID, UINT8 ChannelID, unsigned int Command, unsigned int *Data, unsigned int Length)
{
	...
	if (Command == CMD_SELECT_PACKAGE || 
		Command == CMD_DESELECT_PACKAGE || 
		Command == CMD_SET_NCSI_FLOW_CONTROL)
		ChannelID = 0x1f;
...
{
	SET_NCSI_FLOW_CONTROL_REQ_PKT Pkt;
	SET_NCSI_FLOW_CONTROL_RES_PKT *ResPkt;
	...
	/* Check for valid Flow Control value*/
	if ((FlowControl > 3) || (FlowControl < 0))
	{
		printk("Improper Flow Control value\n");
		return NCSI_ERR_FAILED;
	}
	/* Sleep for response or timeout */
	wait_event_interruptible_timeout(NCSI_Process, (info->ProcessPending == 0), NCSI_TIMEOUT);
	...
	Channel = MK_CH_ID(PackageID,0x1F);
	Pkt.FlowControl = FlowControl;
	/* Form NCSI Packet and send */
	FormNCSIPacket(info,CMD_SET_NCSI_FLOW_CONTROL,Channel,PayLoad,cs_offset);
	info->SendLen = sizeof(Pkt);
	retval = SendNCSICommand (info);
	if (retval != NCSI_ERR_SUCCESS)
		goto exit;
	/* Validate Response pkt */
	ResPkt = (SET_NCSI_FLOW_CONTROL_RES_PKT *)info->RecvData;
	if (info->RecvLen != sizeof(SET_NCSI_FLOW_CONTROL_RES_PKT))
	{
		printk("NCSI(%s): Expected Response Size = %zu Got %d\n",info->dev->name,
			sizeof(SET_NCSI_FLOW_CONTROL_RES_PKT),info->RecvLen);
		retval = NCSI_ERR_RESPONSE;
		goto exit;
	}
	/* Validate Response checksum */
	...
	wake_up_interruptible(&NCSI_Process);
	if(retval == NCSI_ERR_SUCCESS)
		printk("NC-SI Set Flow Control Command SUCCESS");
	return retval;
}
