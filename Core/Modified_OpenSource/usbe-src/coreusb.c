...
static u8 VirtualDeviceStatus=0;
#endif
...
UsbWriteData(uint8 DevNo, uint8 ep, uint8 *Buffer, uint32 Len, uint8 ZeroLenPkt)
{
	...
	TDBG_FLAGGED(usbe, DEBUG_CORE,"UsbWriteData():Sending Data of Length %d bytes to ep#%d\n",Len,ep);
	...
	if ((EpInfo->TxData != NULL)|| (EpInfo->TxReady != 1))
	{
		...
		printk("UsbWriteData():TxData=0x%lx DataLen=0x%x DataPos=0x%x Zero=%d Ready=%d\n",
							(unsigned long)EpInfo->TxData,
...
//        timeout = TRANSMIT_TIMEOUT_HID;
	...
	if (Ret == 0)
	{
		TCRIT("ERROR: UsbWriteData(): Transmission timeout for Device %d EP %d for DataSize 0x%X\n",DevNo,ep,EpInfo->TxDataLen);
		EpInfo->TxData  = NULL;
		EpInfo->TxReady = 1;
...
UsbDeviceIntr(uint8 DevNo, uint8 ep)
{
	...
	return (uint)(DevInfo[DevNo].UsbDevice.UsbHwIntr(DevNo,ep));
}
...
UsbRestoreInitState(uint8 DevNo)
{
	...
	if (MAX_USB_HW < DevNo)
	{
		TCRIT ("UsbRestoreInitState(): OUT OF RANGE: dev %d\n", DevNo);
		return;
	}
...
UsbRxHandler0(uint8 DevNo, uint8 *RxData0, uint32 DataLen, uint8 SetupPkt)
{
	...
	TDBG_FLAGGED(usbe, DEBUG_CORE,"UsbRxHandler0():EP0 Received OUT Packet of length %d \n",DataLen);
...
HandleTxData(uint8 DevNo, uint8 ep, ENDPOINT_INFO *EpInfo)
{
	...
	if ((EpInfo->TxDataLen - EpInfo->TxDataPos) >= MaxSize)
	{
		...
		if (EpInfo->TxData == NULL)
		{
			TDBG_FLAGGED(usbe, DEBUG_CORE,"HandleTxData():EOT:Full Pkt Len = (%d) for EP%d\n",Len,ep);
		}
		else
		{
			TDBG_FLAGGED(usbe, DEBUG_CORE,"HandleTxData(): Full Pkt Len = (%d) for EP%d\n",Len,ep);
		}
	...
	if (EpInfo->TxDataLen != EpInfo->TxDataPos)
	{
		...
		TDBG_FLAGGED(usbe, DEBUG_CORE,"HandleTxData():EOT:Partial Pkt Len = (%d) for EP%d\n",Len,ep);
...
UsbConfigureFS(uint8 DeviceNo)
{
    ...
    if (IsDevHid(DeviceNo))
    {
        /* For HID OS string Descriptor requirement */
        DevDesc->bcdUSBL 			= 0x00;
        DevDesc->bcdUSBH 			= 0x02;	/* USB 2.0 */
    }
    else
...
UsbWriteDataNonBlock(uint8 DevNo, uint8 ep, uint8 *Buffer, uint32 Len, uint8 ZeroLenPkt)
{
	...
	if ((EpInfo->TxData != NULL)|| (EpInfo->TxReady != 1))
	{
		...
		printk("UsbWriteDataNonBlock():TxData=0x%lx DataLen=0x%x DataPos=0x%x Zero=%d Ready=%d\n",
		       (unsigned long)EpInfo->TxData,
		       EpInfo->TxDataLen,EpInfo->TxDataPos,
		       EpInfo->TxZeroLenPkt,EpInfo->TxReady);
	}
...
int
UsbUpstreamDisable_Device_all ()
{
	uint8 DevNo;

	if(VirtualDeviceStatus == 0)
		return 0;

	for (DevNo = 0; DevNo < MAX_USB_HW; DevNo++) {
		if (DevInfo[DevNo].UsbDevice.UsbHwUpstreamDisable == NULL)
					continue;
		DevInfo[DevNo].UsbDevice.UsbHwUpstreamDisable(DevNo);
	}
	VirtualDeviceStatus = 0;
	return 0;
}

int
UsbUpstreamEnable_Device_all ()
{
	uint8 DevNo;

	if(VirtualDeviceStatus == 1)
		return 0;

	for (DevNo = 0; DevNo < MAX_USB_HW; DevNo++) {
		if (DevInfo[DevNo].UsbDevice.UsbHwUpstreamEnable == NULL)
					continue;
		DevInfo[DevNo].UsbDevice.UsbHwUpstreamEnable(DevNo);
	}
	VirtualDeviceStatus = 1;
	return 0;
}

int UsbUpstreamGet_Device_all_status(IUSB_IOCTL_DEVICE_STATUS *Status)
{
	u8 *virtualstat;
	
	virtualstat = (u8 *)&VirtualDeviceStatus;
	if (__copy_to_user((void *)Status,(void *)virtualstat,sizeof (IUSB_IOCTL_DEVICE_STATUS)))
		{
...
// Added support for disable USB 2.0
int
UsbUpstreamDisable_MediaDevice ()
{
	uint8 DevNo;
	for (DevNo = 0; DevNo < MAX_USB_HW; DevNo++) {
		if (DevInfo[DevNo].UsbDevice.UsbHwUpstreamMediaDisable == NULL)
					continue;
		DevInfo[DevNo].UsbDevice.UsbHwUpstreamMediaDisable(DevNo);
	}
	return 0;
}

// Added support for enable USB 2.0
int
UsbUpstreamEnable_MediaDevice ()
{
	uint8 DevNo;
	for (DevNo = 0; DevNo < MAX_USB_HW; DevNo++) {
		if (DevInfo[DevNo].UsbDevice.UsbHwUpstreamMediaEnable == NULL)
					continue;
		DevInfo[DevNo].UsbDevice.UsbHwUpstreamMediaEnable(DevNo);
	}
	return 0;
}
int USBGetDeviceType(uint8 DevNo)
{
	return(DevInfo[DevNo].UsbDevType);
}

EXPORT_SYMBOL(UsbGetRxData);
EXPORT_SYMBOL(UsbGetRxDataLen);
