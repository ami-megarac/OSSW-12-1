...
static void RndisNotifyHost(void)
{
	...
	epnum = NotifyEpNum [0];
	if (UsbCore.CoreUsbWriteDataNonBlock(DeviceNumber, epnum, (uint8 *)&notification, 8, NO_ZERO_LEN_PKT) == 0)
	{
		if (atomic_read(&RndisRequestPending) > 0)
		{
			atomic_dec(&RndisRequestPending);
		}
	}
	else
	{
		TCRIT ("RndisNotifyHost() : Error sending the packet for DevNo %d epnum %d\n", DeviceNumber, epnum);
	}
}
...
void EthTxHandler(uint8 DevNo,uint8 ifnum, uint8 epnum)
{
	...
	{
		if( (atomic_read(&RndisRequestPending) > 0) && (atomic_dec_return(&RndisRequestPending) > 0) )
		{
			RndisNotifyHost();
		}
		return;
...
static inline void SendUsbData(uint8 *Data, uint16 Length, uint8 ZeroLenPkt)
{
	...
	{
		NET_PACKET *Pkt;
		TDBG ("SendUsbData() : Error sending the packet for DevNo %d epnum %d\n", DeviceNumber, epnum);
		spin_lock_irqsave(&Lock, flags);
		if ((epnum != 0) && (Current))
		{
			Pkt = Current;
...
int EthReqHandler(uint8 DevNo,uint8 ifnum, DEVICE_REQUEST *pRequest)
{
	...
	case CDC_GET_ETHERNET_STATISTIC:
		TDBG("EthReqHandler(): Unimplemented CDC request : %d\n", pRequest->bRequest);
		return 1;
