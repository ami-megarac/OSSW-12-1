...
static gpio_hal_operations_t gpio_hw_ops = {
	...
	NULL,	// get_debounce_event
	NULL,	// set_debounce_event
...
{
 	if(pgpio_core_funcs->gpio_verify(gpio_num) == 1)
 	    return 1;
	return 0;
}
...
#endif
						intr_type = pgpio_core_funcs->process_gpio_intr(i, 0, 0, NOT_LEVEL_TRIGGER);
					}
				}
...
#endif
						intr_type = pgpio_core_funcs->process_gpio_intr(i, 32, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
...
#endif
						intr_type = pgpio_core_funcs->process_gpio_intr(i, 64, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
...
#endif
						intr_type = pgpio_core_funcs->process_gpio_intr(i, 96, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
...
#endif
						intr_type = pgpio_core_funcs->process_gpio_intr(i, 128, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
			...
			for(i = 0; i <= GPIO_31; i++)
			{
				if(intr_state_ast2050_value & 0x00000001)
				{
					...
					if(ast2050_gpio_verify(gpio_num)) 
					{
						...
						current_ast2050_intrs |= value;
						//common module's callback interrupt handler
						intr_type = pgpio_core_funcs->process_gpio_intr(i, 160, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
			...
			for(i = 0; i <= GPIO_31; i++)
			{
				if(intr_state_ast2050_value & 0x00000001)
				{
					...
					if(ast2050_gpio_verify(gpio_num)) 
					{
						...
						current_ast2050_intrs |= value;
						//common module's callback interrupt handler
						intr_type = pgpio_core_funcs->process_gpio_intr(i, 192, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
			...
			for(i = 0; i <= GPIO_31; i++)
			{
				if(intr_state_ast2050_value & 0x00000001)
				{
					...
					if(ast2050_gpio_verify(gpio_num)) 
					{
						...
						current_ast2050_intrs |= value;
						//common module's callback interrupt handler
						intr_type = pgpio_core_funcs->process_gpio_intr(i, SGPIO_START_PIN, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
			...
			for(i = 0; i <= GPIO_31; i++) 
			{
				if(intr_state_ast2050_value & 0x00000001)
				{
					...
					if(ast2050_gpio_verify(gpio_num)) 
					{
						...
						current_ast2050_intrs |= value;
						//common module's callback interrupt handler 
						intr_type = pgpio_core_funcs->process_gpio_intr(i, (BANK_EXTSGPIO_DATA_OFFSET * 8), 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
			...
			for(i = 0; i <= GPIO_31; i++)
            {
				if(intr_state_ast2050_value & 0x00000001)
				{
					...
					if(ast2050_gpio_verify(gpio_num)) 
					{
                        ...
                        current_ast2050_intrs |= value;
                        //common module's callback interrupt handler
                        intr_type = pgpio_core_funcs->process_gpio_intr(i, 288, 0, NOT_LEVEL_TRIGGER);
					}
				}
				intr_state_ast2050_value >>= 1;
			}
...
int ast2050_unregsensorints (  void* gpin_data )
{
	...
	if(gpin_data == NULL)//check for NULL pointer
	    return 0;
	
	bank = (((Gpio_data_t*)gpin_data)->PinNum / 8);
	tpin = (((Gpio_data_t*)gpin_data)->PinNum % 32);
	...
	{	
		...
		tpin = ((((Gpio_data_t*)gpin_data)->PinNum - SGPIO_START_PIN) % 32);
#if defined(GROUP_AST2400) || defined(GROUP_AST2500)
			if(bank >= BANK_EXTSGPIO_1_DATA_OFFSET) {
					int_enable_ast2050_Addr 	= (uint32_t)gpio->ast2050_gpio_addr+EXTSGPIO_1_INT_EN_OFFSET;
					int_riseneg_ast2050_Addr	= (uint32_t)gpio->ast2050_gpio_addr+EXTSGPIO_1_INT_SEN_TYPE0_OFFSET;
