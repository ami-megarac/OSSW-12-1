...
+/*Fixed Timer3 interrupt will let cpu write or erase Flash Environment sector fail issue in bootloader*/
+#if defined(CONFIG_PILOT4)
+	extern void start_pilot_timer3(void);
+	extern void stop_pilot_timer3(void);
+#endif
+
...
+	if(flash_info[bank].flash_id >> 16 == 0x9D)
+	code = CMD_ISSI_EX4B;
+
+	/* Exit 4 byte addresing for spansion chip */
+	if (flash_info[bank].flash_id == 0x00011902) 
+	{
+		code = CMD_S25FLXX_BRWR;
+		/* Bit 7- Extented Address Enable. 1- 4byte(32 bits addresssing), 0 - 3byte(24 bit addressing) */
...
+	/*Fixed Timer3 interrupt will let cpu write or erase Flash Environment sector fail issue in bootloader*/
+	#if defined(CONFIG_PILOT4)
+	unsigned long start_erase_sect_addr = sect_addr;
+		if(start_erase_sect_addr  == CONFIG_SPX_FEATURE_GLOBAL_UBOOT_ENV_START)
+			stop_pilot_timer3();
+	#endif
+
...
+		{
+			printf("spansion unprotect sector unsuccessful\n");
+			return -1;	
+		}
+	}
+
...
+		if (spansion_sector_handler(protect_state, bank, addr_ori, bytes, priv->ctrl_drv) != 0)
+		{
+			printf("spansion protect sector unsuccessful\n");
+			return -1;	
+		}
+	}
+
...
+	/*Fixed Timer3 interrupt will let cpu write or erase Flash Environment sector fail issue in bootloader*/
+	#if defined(CONFIG_PILOT4)
+	unsigned long start_write_addr = addr;
+		if(start_write_addr  == CONFIG_SPX_FEATURE_GLOBAL_UBOOT_ENV_START)
+			stop_pilot_timer3();
+	#endif
+
+	/* For Spantion SPI  */
...
+
+	  	ctrl_drv->spi_transfer(bank,command_sec, 5,SPI_READ,&dyb, 1,0);
...
+    		command_sec[0] = 0xe1;
+		spi_generic_write_enable(bank,ctrl_drv); /* Send write enable */
+		retval = ctrl_drv->spi_transfer(bank,command_sec, 6 ,SPI_NONE, NULL, 0,0);
+		if(retval < 0)
+		return -1;
+		spi_generic_write_disable(bank,ctrl_drv); /* Send write disable */
+	}
...
+		extadd = 0x0;
+	}
+	retval = ctrl_drv->spi_transfer(bank,&brwr_code, 1,SPI_WRITE, &extadd, 1, 0);//enable extended address
+	if(retval < 0)
+		return -1;
+	return 0;
+}
