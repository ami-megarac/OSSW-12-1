...
#define ETH_USB_MAC_FILE "/conf/eth_usb_mac"
...
init_eth_device_module (void)
{
	...
	int devnum;
	struct file *f=NULL;
	uint8 hostmac[ETH_ALEN] = {0};
	mm_segment_t fs;
	
	f = filp_open(ETH_USB_MAC_FILE, O_RDWR|O_CREAT, 0644);
	if(f != NULL)
		{
			fs = get_fs();		// Get current segment descriptor
			set_fs(get_ds());	// Set segment descriptor associated to kernel space
			ret_val = f->f_op->read(f, hostmac, sizeof(hostmac), &f->f_pos);
			//printk("Read Bytes: %d\n",ret_val);
			if(ret_val == 0)
			{
				random_ether_addr(hostmac);
				f->f_op->write(f, hostmac, sizeof(hostmac), &f->f_pos);
			}
			printk("Ethernet Device Hostmac is %x:%x:%x:%x:%x:%x \n",hostmac[0],hostmac[1],hostmac[2],hostmac[3],hostmac[4],hostmac[5]);
			set_fs(fs);			// Restore segment descriptor
			filp_close(f,NULL);
		}
		else
		{
			printk("Failed to open %s file\n",ETH_USB_MAC_FILE);
			random_ether_addr(hostmac);
		}
